...
function generatePdfContent(data: ChecklistFormData): string {
  ...
  content += `Carbon Monoxide Test (gas appliances): ${data.section2.carbonMonoxideTest || "N/A"}\n\n`;

  content += "3. KITCHEN/BATHROOM INSPECTION (8-9 Minutes)\n";
  content += `[${data.section3.hotWaterFixtures.checked ? "X" : " "}] Hot Water - Temp: ${data.section3.hotWaterFixtures.waterTemp || "N/A"}°F\n`;
  content += `Hot Water Arrival Time: ${data.section3.hotWaterArrivalTime || "N/A"}\n`;
  content += `[${data.section3.drainage ? "X" : " "}] Drainage tested\n`;
  content += `[${data.section3.popUpAssembly ? "X" : " "}] Pop-Up Assembly verified\n`;
  content += `[${data.section3.soapDispenser ? "X" : " "}] Soap dispenser tested - If applicable\n`;
  content += `[${data.section3.disposalCheck ? "X" : " "}] Check Disposal for proper function/cracks in the housing/leaks\n`;
  ...
  content += `[${data.section4.behindRefrigeratorDry ? "X" : " "}] Behind refrigerator dry and free of mold - If Applicable\n`;
  content += `[${data.section4.waterHeaterAreaDry ? "X" : " "}] Water heater area dry - If applicable\n`;
